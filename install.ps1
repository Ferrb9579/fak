...
try {
    ...
    try {
        # Keep the PowerShell hook generated by the binary so `fak --alias`
        # and the installer cannot drift apart.
        $env:FAK_SHELL = "powershell"
        $generatedHook = (& $binaryPath --alias) -join [Environment]::NewLine
    } finally {
        if ($null -eq $oldShell) { Remove-Item Env:FAK_SHELL -ErrorAction SilentlyContinue } else { $env:FAK_SHELL = $oldShell }
    }
    $hook = "# >>> fak shell integration >>>`n$generatedHook`n# <<< fak shell integration <<<"
